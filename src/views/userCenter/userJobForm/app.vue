...
export default {
  data () {
    return {
      ...
      form: {
        ...
        starTime: window.api && window.api.pageParam.job ? window.api.pageParam.job.starTime : Date.now().valueOf(),
  ...
  methods: {
    async create () {
      ...
      // }));
      tools.showProgress();
      const response = await service.createUserJob({
        ...this.form,
        resumeId: window.api.pageParam.resumeId
      });
      tools.hideProgress();
